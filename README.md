...
- **Download Warehouse with metadata** [CSV](https://drive.google.com/file/d/1YG9N0HF1m7qI4za1qyS-ckTfQZvQ1Ipw/view?usp=drive_link)
- **Download Warehouse without metadata** [CSV](https://drive.google.com/file/d/19XLHMezkzPng1kdCsDTj1VR1PVjkQTVX/view?usp=drive_link)
...
- **Download the Wealth Topography database**: [CSV](https://drive.google.com/file/d/19vzSCntJB4e8hDFBgd0ZG_9C1J1OzrSH/view?usp=drive_link)
...
- **Download the Wealth Inequality Trends database**: [CSV](https://drive.google.com/file/d/1myY2OKWAXhr_ipRY1rTfGBr3-3dmTD_I/view?usp=drive_link)
...
- **Download the supplementary variables (Jul 16, 2024)** [CSV](https://drive.google.com/file/d/1TVVDszPqSZ__o4F_X8WOlWn3t2HbdcjW/view?usp=drive_link)
...
Source metadata is managed from `code/tools/metadata/sources/sources.yaml` (JSON-compatible YAML).

- Add/edit requests should use the local Source Registry UI: `python3 code/tools/sources/ui_local.py`.
- The UI has one editable `Library` tab. Each record can be marked as a data source, which reveals data-source-only fields and the controlled data-source category checkboxes. Research categories are also controlled checkboxes stored as BibTeX `keywords`.
...
- `documentation/BibTeX files/digital_library.bib` is the single public BibTeX export generated from the canonical registry.
- `handmade_tables/dictionary.xlsx` (`Sources` and `SourceAliases` sheets) is generated from records marked as data sources; do not edit it manually.
- `Source` is the warehouse/dictionary code for data sources only, while `citekey` is the BibTeX identifier for every library record. Collapsed `_ineq`/`_topo` sources use extensionless `Source` values such as `LWS` or `CS`, with the old suffixed codes stored in `SourceAliases`.
- The mainstream pipeline uses `SourceAliases` to canonicalize old suffixed dashboard source names, so extensionless names like `LWS` and `WID` are no longer hard-dropped.
- Legacy split BibTeX files are archived under `documentation/BibTeX files/old/` and are not generated as active outputs.
- Active audit entries are stored in `code/tools/metadata/sources/change_log.yaml`; legacy Wealth Research history remains readable where needed.
- Validation and generation scripts live under `code/tools/sources/`.
- Workflow reference: `code/tools/documentation/workflow/sources/source_registry.md`.
...
- Full data warehouse: [download](https://drive.google.com/file/d/1FmO8qVbBshI0iQqyY321jjp1jfKnvLT_/view?usp=share_link)
