***********************************
*** taxw US states translation code
***********************************

// Last update: March 2026

// Data used: $intfile/taxw_USstates_taxsched_data.dta; $intfile/taxw_USstates_sources; taxw_USstates_revenue_data
// Output: $intfile/taxw_USstates_taxsched_data_transformed.dta; $intfile/taxw_USstates_revenue_transformed; /// 
// 		   $intfile/taxw_USstates_transformed 

// Load data 
	qui use "$intfile/taxw_USstates_taxsched_data.dta", clear
	compress
********************************************************************************
*** TAX SCHEDULE
****************
...
// Attach source 
	qui merge m:1 GeoReg year using "$intfile/taxw_USstates_sources.dta", keep(master matched)
	qui: count if _m != 3
	if (`r(N)' != 0) {
...
// Generate new variables (even if empty for regional data)	
	gen homexe = -999
	gen bssexe = -999
	gen diftax = -999
	
	label define labels -999 "Missing" -998 "_na" -997 "_and_over"
	foreach var in exempt toprat toplbo adjlbo adjubo adjmrt homexe bssexe diftax {
...
// Save 
	...
	qui save "$intfile/taxw_USstates_taxsched_transformed.dta", replace
...
// Load data 
	use "$intfile/taxw_USstates_revenue_data.dta", clear
...
// Attach source 
	merge m:1 GeoReg year using "$intfile/taxw_USstates_sources.dta", keep(master matched)
	qui: count if _m != 3
	if (`r(N)' != 0) {
		display in red "`r(N)' Unmatched sources, check"
	...
	save "$intfile/taxw_USstates_revenue_transformed.dta", replace

********************************************************************************
*** taxw data: append data
***************************

// Load tax schedule data
	qui use "$intfile/taxw_USstates_taxsched_transformed.dta", clear
...
// Attach revenue data	
	qui append using "$intfile/taxw_USstates_revenue_transformed.dta"
	sort GeoReg year tax br applies_to
	order GEO* GeoReg* year tax appl br adjlb adjub adjmr statu exemp topra toplb homexe bssexe diftax revenu prorev revgdp
...
// Generate new variable type of tax
	...
	order GEO GEO_long GeoReg GeoReg_long year tax applies_to bracket adjlbo adjubo adjmrt status typtax 
	...
	save "$intfile/taxw_USstates_transformed.dta", replace
