// Sources harmonization for Canada

	use "$sources/Final_Data/CA/appended_CA", clear
	...
	// Overlapping for all taxes (everybody) from 2006 to 2024: we have legislative information overlapping with EYa and b, so we keep the legislative information.

	destring year_from, replace
	drop if year_from >= 2006
	tostring year_from, gen(pippo)
	drop year_from 
	rename pippo year_from
	order GEO* curr year_f year_t
	
	drop if Source == "TIDData" // redundand, unknown applies to
	replace typetax = "4" if status == "1"
	
	// Overlapping in 1972 for estate tax between Lex Canada and Yale. We keep Law as superior source 
	drop if year_from == "1972" & year_to == "1972" & Source == "YaleInheritanceData" & tax == "estate"	
	
	// Overlapping for inheritance between 1972 and 2009 between Lex Canada and Yale. We keep Law as superior source from 1972, while Yale up to 1971
	replace year_to = "1971" if year_to == "2009" & tax == "inheritance" & applies_to == "everybody" & Source == "YaleInheritanceData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/CA/Final_CA.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/CA/appended_CA.dta"	