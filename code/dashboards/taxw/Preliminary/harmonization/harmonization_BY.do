// Sources harmonization for Belarus

	use "$sources/Final_Data/BY/appended_BY", clear
		...
		// Overlapping in 2008 between TIDD and EY. We keep TIDD until 2007. 
		replace year_to = "2007" if Source == "TIDData"
		
		// Adjust currency
		replace currency = "BYN" if currency == "BY"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/BY/Final_BY.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/BY/appended_BY.dta"