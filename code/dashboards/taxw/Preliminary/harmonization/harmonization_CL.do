use "$sources/Final_Data/CL/appended_CL", clear
		...
		// Overlapping between TIDD, EY and Chile Tax Law. We keep the law that has full coverage since its introduction
		drop if substr(Source, 1,5) != "Chile"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/CL/Final_CL.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/CL/appended_CL.dta"