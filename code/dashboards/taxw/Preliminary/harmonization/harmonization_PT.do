use "$sources/Final_Data/PT/appended_PT", clear
	...
	// Overlapping between Shultz and TIDD in 1844 for non-relatives + siblings in inheritance. We keep Shultz for more complete information
		drop if year_from == "1844" & year_to == "1844" & tax == "inheritance" & applies_to == "siblings" & Source == "TIDData" 
		drop if year_from == "1844" & year_to == "1844" & tax == "inheritance" & applies_to == "non relatives" & Source == "TIDData" 

	// Overlapping between Shultz and TIDD in 1844 for children in inheritance (TID reports staus no and Shultz full exemption for children. We keep Shultz for more complete information
		replace applies_to = "spouse" if year_from == "1844" & year_to == "1844" & tax == "inheritance" & applies_to == "spouse, children" & Source == "TIDData"
		
	// Conflict between Shultz and TIDD in 1844 for other relatives in inheritance (TID reports toprate at 10, Shultz at 5). However, TID reports 10 for relatives beyond 4th degree, Shultz reports 5 for those up to the 4th. We keep Shultz and specify in the note
		drop if year_from == "1844" & year_to == "1844" & tax == "inheritance" & applies_to == "other relatives" & Source == "TIDData"
	...
	// Overlapping between TIDD and Shultz between 1838 and 1843 for inheritance. We keep TIDD as report the rates and kinship.  
		drop if year_from == "1838" & year_to == "1843" & tax == "inheritance" & applies_to == "everybody" & Source == "Shultz1926"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
		...
		export excel using "$sources/Final_Data/PT/Final_PT.xlsx", firstrow(variables) sheet(Data) replace	
		erase "$sources/Final_Data/PT/appended_PT.dta"	
