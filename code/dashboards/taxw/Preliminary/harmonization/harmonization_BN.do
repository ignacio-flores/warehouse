// Sources harmonization for Brunei
	
	
	use "$sources/Final_Data/BN/appended_BN", clear
	...
	// Overlapping TIDD EYa between 2006-2012 unknown-estate. We keep TIDD until 2005 (i.e., before EYa) and then EYa 
		replace year_to = "2005" if year_to == "2012" & year_from == "1984"
		drop if year_to == "2013" & year_from == "2013" & Source == "TIDData"
		
	// Overlapping between TIDD and Brunei tax law for 1984-2005 estate everybody. We keep the law 
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
		...
		export excel using "$sources/Final_Data/BN/Final_BN.xlsx", firstrow(variables) sheet(Data) replace	
		erase "$sources/Final_Data/BN/appended_BN.dta"	
