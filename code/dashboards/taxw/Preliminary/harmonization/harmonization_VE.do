use "$sources/Final_Data/VE/appended_VE", clear
		...
		// Overlapping EYa lex 2006-2023, all taxes. We keep lex as more complete
	...
	* No overlapping between children (or other) and everybody	
	...
	// Further possible overlapping: children and unknown: we keep children because the source is more specific
		...
		// Overlapping between TIDD and lex for inheritance in 2015. TIDD does not report any info, we keep lex
		drop if year_from == "2015" & year_to == "2015" & tax == "inheritance" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/VE/Final_VE.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/VE/appended_VE.dta"