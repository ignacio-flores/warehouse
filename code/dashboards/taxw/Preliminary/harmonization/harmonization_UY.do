use "$sources/Final_Data/UY/appended_UY", clear
	...
	* No overlapping between children (or other) and everybody	
	...
	// Last possible overlapping: everybody and unknown
		...
		// Conflict between TIDD and EYa for gift in 2008. EY reports status 0 and different tax, while TIDD status 1. However, both EY and IBFD reports is a transfer tax. Therefore, we keep EY. 
		drop if tax == "gift" & year_from == "2008" & year_to == "2008" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/UY/Final_UY.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/UY/appended_UY.dta"