use "$sources/Final_Data/RO/appended_RO", clear
	...
	// Overlapping between 1886-1900 for children-inheritance for TIDD and Shultz. We keep Shultz as reliable historical source. 
		replace year_to = "1885" if year_to == "1899" & year_from == "1877" & Source == "TIDData" & tax == "inheritance" 
		drop if year_to == "1900" & year_from == "1900" & tax == "inheritance" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/RO/Final_RO.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/RO/appended_RO.dta"		
