use "$sources/Final_Data/GN/appended_GN", clear
		...
		// Overlapping EY IBFD inheritance and estate tax 2010-2023, we keep IBFD
		drop if year_from == "2022" & substr(Source, 1,2) == "EY"
		replace year_to = "2009" if year_to == "2021" & substr(Source, 1,2) == "EY"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/GN/Final_GN.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/GN/appended_GN.dta"