// Sources harmonization for Azerbaijan

	use "$sources/Final_Data/AZ/appended_AZ", clear
	...
	// Overlapping between TIDD and EY for inheritance-everybody for the years 2006-2023. We keep EY. 
		replace year_to = "2005" if year_to == "2021" & tax == "inheritance" & applies_to == "everybody" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/AZ/Final_AZ.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/AZ/appended_AZ.dta"