// Sources harmonization for Armenia

	use "$sources/Final_Data/AM/appended_AM", clear
	...
	// Overlapping for inheritance tax, everybody, between 2010 and 2023 for TIDD and EY. We keep EY in those years (richer in notes)
		replace year_to = "2009" if year_to == "2021" &  Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/AM/Final_AM.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/AM/appended_AM.dta"