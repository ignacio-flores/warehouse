// Sources harmonization for Dominical Republic

	use "$sources/Final_Data/DO/appended_DO", clear
	...
	* No overlapping between children (or other) and everybody	
	...
	// Overlapping lex and TID 1950-2003, we keep lex.
	replace year_to = "1949" if year_to == "2003" & Source == "TIDData"
	...
	// Overlapping lex and TID 2004, we keep lex.
	drop if year_to == "2004" & Source == "TIDData"
	
	// Overlapping lex and EYa 2006-2023, we keep lex.
	drop if substr(Source,1,2) == "EY"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/DO/Final_DO.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/DO/appended_DO.dta"