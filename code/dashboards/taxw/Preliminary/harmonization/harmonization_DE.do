// Sources harmonization for Germany

	use "$sources/Final_Data/DE/appended_DE", clear
	...
	// Overlapping Yale and lex for 1959-1973 of inheritance tax for children. We keep the law with the schedule
	replace year_to = "1950" if year_to == "1954" & Source == "YaleInheritanceData" & tax == "inheritance" & applies_to == "children"	
	// Overlapping Yale and lex for 1951-2008 of estate tax for everybody. We keep the law
	replace year_to = "1950" if year_to == "2008" & Source == "YaleInheritanceData" & tax == "estate" & applies_to == "everybody"		
	
	// From 1951 on, we keep the tax law.	
	destring year_from, replace
	drop if year_from >= 1951 & substr(Source, 1, 6) != "German" 
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/DE/Final_DE.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/DE/appended_DE.dta"	