// Sources harmonization for Bulgaria

	use "$sources/Final_Data/BG/appended_BG", clear
	...
	* No overlapping between children (or other) and everybody	
	...
	// Last possible overlapping: everybody and unknown
		...
		// Conflict between TIDD and EYa for inheritance in 2018. TIDD reports status 1 to unknown, while EY status 0 to everybody. We know Bulgaria applies a municipal tax, so drop TIDD that do not report subnational level. 
		drop if year_from == "2018" & year_to == "2018" & applies_to == "unknown" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/BG/Final_BG.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/BG/appended_BG.dta"