use "$sources/Final_Data/FJ/appended_FJ", clear
	...
	// Overlapping between EYa and TID for all taxes, status no, 2006-2023. We keep EY.
	replace year_to = "2005" if year_from == "1970" & year_to == "2021" & Source == "TIDData"
	drop if year_from == "2021" & year_to == "2021" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/FJ/Final_FJ.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/FJ/appended_FJ.dta"		
