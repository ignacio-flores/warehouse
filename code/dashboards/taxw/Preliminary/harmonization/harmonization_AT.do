// Sources harmonization for Austria

	use "$sources/Final_Data/AT/appended_AT", clear
	...
	// Overlapping information 1850-1915 for children inheritance tax from Shultz1926 and YaleInheritanceData: same info apart from year 1863 in which Yale is more precise 
	drop if Source == "Shultz1926" & year_from == "1850" & year_to == "1861" & applies_to =="children" & tax == "inheritance"
	drop if Source == "Shultz1926" & year_from == "1862" & year_to == "1915" & applies_to =="children" & tax == "inheritance"

	// Overlapping information for children-inheritance between 1955 and 2005 for Yale vs Austrian tax law. We keep Austrian tax law as it display full schedule. 	
	replace year_to = "1954" if year_to == "1967" & (tax == "inheritance") & applies_to == "children" & Source == "YaleInheritanceData"
	replace year_to = "1954" if year_to == "2008" & (tax == "estate") & applies_to == "everybody" & Source == "YaleInheritanceData"
	
	// From 1955 we keep information from the tax law.
	destring year_from, replace 
	drop if year_from >= 1955 & substr(Source, 1, 7) != "Austria"
	tostring year_from, replace	
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/AT/Final_AT.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/AT/appended_AT.dta"