// Sources harmonization for Bolivia

	use "$sources/Final_Data/BO/appended_BO", clear
		...
		// Overlapping in 2008 between TIDD and Lex. We keep Lex. 
		drop if year_to == "2008" & Source == "TIDData"
		
		// Overlapping in 2016-2023 between Lex and EY. We keep Lex. 
		drop if year_to == "2023" & Source == "EY2023a"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/BO/Final_BO.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/BO/appended_BO.dta"