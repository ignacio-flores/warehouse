	use "$sources/Final_Data/IT/appended_IT", clear
	...
	// Overlapping between ItalianMEFInfo and TIDData for the years 1862-1901 inheritance and various kinship. We keep the more complete national law info ItalianMEFInfo.
	drop if year_from == "1862" & year_to == "1901" & Source == "TIDData"
	drop if Source == "TIDData"
	
	destring year_from, replace
	destring year_to, replace
	drop if year_from >= 2006 & year_from <= 2011 & substr(Source, 1, 2) == "EY"
	tostring year_from, gen(pippo)
	drop year_from
	rename pippo year_from
	tostring year_to, gen(pippo)
	drop year_to
	rename pippo year_to	
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/IT/Final_IT.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/IT/appended_IT.dta"	
