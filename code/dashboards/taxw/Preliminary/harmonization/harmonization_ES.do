use "$sources/Final_Data/ES/appended_ES", clear
	...
	// Overlapping between Law and EY between 2012 to 2024. We keep the superior source i.e., Law 
	destring year_from, gen(p_from)
	destring year_to, gen(p_to)
	drop if substr(Source,1,2)=="EY" & p_from >=2012 & p_from <= 2024 & p_to >= 2012 & p_to <= 2024 
	replace note = "Regions might modify the schedule established by the central government. " + note if p_from >= 1996 
	drop p_*
	
	// TIDD reports the correct independence year (1814), so we drop info from Shultz before that year, and replace its firsty 
	drop if year_from == "1792" & year_to == "1808" 
	replace firsty = "1814" if firsty == "1792"
	drop if year_from == "1821" & year_to == "1822" & Source == "Shultz1926" & applies_to == "unknown"
	...
	* No overlapping between children (or other) and everybody	
	...
	// Further possible overlapping: children and unknown: we keep children because the source is more specific
		...
		// Overlapping between Shultz and TIDD between 1814-1844 for non-relativs vs unknow in inheritance. From 1845 to 1914 overlapping between unknow and all kinships. 
			// Therefore, we keep Shultz from 1829 onwards. 
			replace year_to = "1828" if year_to == "1914" & year_from == "1814" & applies_to == "unknown" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/ES/Final_ES.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/ES/appended_ES.dta"	