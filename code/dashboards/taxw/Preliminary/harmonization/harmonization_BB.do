// Sources harmonization for Barbados

	use "$sources/Final_Data/BB/appended_BB", clear
		...
		// Overlapping in 2008 for everybody-estate in TIDD and EY. We keep EY 
		drop if Source == "TIDData" & tax == "estate" & applies_to == "everybody" & year_from == "2008"
	...
	// Last possible overlapping: everybody and unknown
		...
		// Conflict in status for gift between TIDD and EY. EY is correct one 
		drop if Source == "TIDData" & tax == "gift" & year_from == "2008"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/BB/Final_BB.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/BB/appended_BB.dta"