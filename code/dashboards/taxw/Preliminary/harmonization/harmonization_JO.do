	use "$sources/Final_Data/JO/appended_JO", clear
...
// Overlapping between TID and EY 2006-2023 for all taxes (status no). We drop EY as no additional notes are present
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/JO/Final_JO.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/JO/appended_JO.dta"	
