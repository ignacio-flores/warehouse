	// Harmonization file for Ecuador	
		
		use "$sources/Final_Data/EC/appended_EC", clear
	...
	// Overlapping for unknown inheritance between TIDD and EY. EY is not correct for 2007-08. We keep TID
	drop if year_from == "2007" & year_to == "2008" & tax == "inheritance" & applies_to == "unknown" & substr(Source, 1,2) == "EY"
	replace typetax = "4" if year_from == "2007" & year_to == "2008" & Source == "TIDData" & tax == "inheritance" & applies_to == "unknown"
	
	// Overlapping TID EY for inheritance in 2006. We stop TID in 2005. 
	replace year_to = "2005" if year_to == "2006" & tax == "inheritance" & applies_to == "unknown" & Source == "TIDData"
	
	// Overlapping IBFD EY for estate 2009-2023, we keep ibfd 
	drop if tax == "estate" & substr(Source, 1, 2) == "EY"
	...
	* No overlapping between children (or other) and everybody	
	...
	// Overlapping EY IBFD 2009-2023 for gift and inheritance, we keep IBFD
	destring year_from, replace
	drop if substr(Source,1,2) == "EY" & year_from >= 2009
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/EC/Final_EC.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/EC/appended_EC.dta"		
