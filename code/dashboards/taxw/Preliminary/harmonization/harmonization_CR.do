// Sources harmonization for Costa Rica

	use "$sources/Final_Data/CR/appended_CR", clear
	...
	// Overlapping information of no estate, inheritance and gift tax in 2008, but EYa has info from 2006 to 2023, so we keep it but with the note from TID (more complete). 
	
	drop if year_from == "2008" & Source == "TIDData"
	replace note = "Costa Rica does not have inheritance, gift, estate or succession taxes, but a tax on transfer of real property. By 2008, the rates are between 1 and 2% and divisions between spouses and joint owners are not taxable"	if Source == "EY2023a" 
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/CR/Final_CR.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/CR/appended_CR.dta"	