// Sources harmonization for Australia

	use "$sources/Final_Data/AU/appended_AU", clear
		...
		// Overlapping everybody estate tax between 2012-2019 for EY vs TIDD. We keep EY since 2012 as more detailed/reliable. 
		replace year_to = "2011" if year_to == "2019" & tax == "estate" & applies_to == "everybody" & Source == "TIDData"
	...
	// Overlapping between TIDD and Yale in 1979 for estate children. 
		replace year_from = "1980" if year_from == "1979" & tax == "estate" & applies_to == "everybody" & Source == "TIDData"
	...
	// Further possible overlapping: children and unknown: we keep children because the source is more specific
		...
		// Overlapping between TIDD and Yale for children-unknown inheritance tax between 1914 and 1978. We keep Yale as more informative. 
		drop if year_from == "1914" & year_to == "1978" & tax == "estate" & applies_to == "unknown" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/AU/Final_AU.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/AU/appended_AU.dta"