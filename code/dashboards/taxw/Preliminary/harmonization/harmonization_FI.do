// Sources harmonization for Finland

	use "$sources/Final_Data/FI/appended_FI", clear
	...
	// Yale and EYa reporting the same info for years 2006-2009, but EY having full shcedule and Yale having first year.
	drop if Source == "YaleInheritanceData" & tax == "inheritance" & applies_to == "children" & year_from == "2008" & year_to == "2009"
	replace year_to = "2005" if year_to == "2007" & Source == "YaleInheritanceData" & tax == "inheritance" & applies_to == "children"
	
	// Overlapping between EY and Yale, same status 0, as we have kept EY, we drop Yale. 
	replace year_to = "2005" if year_to == "2009" & year_from == "2002" & tax == "estate" & applies_to == "everybody" & Source == "YaleInheritanceData"
	...
	// Further possible overlapping: children and unknown: we keep children because the source is more specific
		...
		// Overlapping for inheritance-unkown of TIDD with Yale-children (and EY) from 1919 to 2009. We drop TIDD 
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/FI/Final_FI.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/FI/appended_FI.dta"