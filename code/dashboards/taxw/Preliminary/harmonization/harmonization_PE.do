use "$sources/Final_Data/PE/appended_PE", clear
		...
		// Overlapping for estate, inheritance and gift between tax law and some information from TID nad EY. The tax law is always more informative
		drop if substr(Source, 1, 4) != "Peru"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
		...
		export excel using "$sources/Final_Data/PE/Final_PE.xlsx", firstrow(variables) sheet(Data) replace	
		erase "$sources/Final_Data/PE/appended_PE.dta"	
