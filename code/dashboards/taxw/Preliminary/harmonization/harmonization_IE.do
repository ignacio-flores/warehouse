	use "$sources/Final_Data/IE/appended_IE", clear
	...
	// Overlapping between IE Law and other sources over time. As we cover the whole period with the Law, we keep the law. 
	drop if substr(Source, 1,2) != "IE"
	replace applies_to = "non relatives" if applies_to == "other relatives"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/IE/Final_IE.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/IE/appended_IE.dta"	
