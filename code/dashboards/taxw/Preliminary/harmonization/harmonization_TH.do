use "$sources/Final_Data/TH/appended_TH", clear
	...
	/* Overlapping information between EYa and TIDD between 2006 and 2011. However, TIDD is able to cover the whole period for inheritance. It starts in 1933 up to 2025. 
       On the contrary, EYa reports information on gifts not available in TIDD. Therefore, we keep TIDD for inheritance, but EY for gifts */ 
	   drop if tax == "inheritance" & substr(Source, 1,2) == "EY"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
		...
		export excel using "$sources/Final_Data/TH/Final_TH.xlsx", firstrow(variables) sheet(Data) replace	
		erase "$sources/Final_Data/TH/appended_TH.dta"	
