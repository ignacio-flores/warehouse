	use "$sources/Final_Data/IN/appended_IN", clear
	...
	// Overlapping between EYb and LegislativeHistory and JournalofTA2018 for the years 2004-2018
	replace year_to = "2003" if year_to == "2018" & Source == "JournalofTA2018_India" & year_from =="1985"
	drop if year_from == "2004" & year_to == "2005" & Source == "LegislativeHistory_India"
	drop if year_from == "2006" & year_to == "2017" & Source == "LegislativeHistory_India"
	...
	* No overlapping between children (or other) and everybody	
	...
	// Overlapping TID and legislation, 1953-1986, inheritance. The information in TID is actually wrong, it reports inheritance status = 1 but we know from legislation that it is an estate tax.
	drop if year_from == "1953" & tax == "inheritance" & Source == "TIDData"
	drop if year_from == "1986" & tax == "inheritance" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/IN/Final_IN.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/IN/appended_IN.dta"	
