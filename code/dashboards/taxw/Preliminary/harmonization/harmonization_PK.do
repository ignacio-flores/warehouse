	use "$sources/Final_Data/PK/appended_PK", clear
	...
	// Overlapping EYa and IBFD, gift tax 2006-2018, we keep IBFD
	drop if year_from == "2006" & year_to == "2018" & substr(Source,1,2) == "EY" & tax == "gift"
	...
	* No overlapping between children (or other) and everybody	
	...
	// Overlapping EY IBFD 2019-2023, gift tax, we keep IBFD but preserve EY info in the note
	drop if year_from == "2019" & year_to == "2023" & tax == "gift" & substr(Source,1,2) == "EY"
	replace note = "We note that according to EY reports for the years 2019-2023 the receipt of gift is included in taxable income unless receieved from immediate family members." if year_from == "1985" & year_to == "2025" & tax == "gift"
	...
	// Overlapping TID and legislation, 1950-1962, estate and gift. 
	replace year_from = "1979" if year_from == "1950" & tax == "estate" & Source == "TIDData" 	
	// Overlapping IBFD and legislation, 1963-1978, estate and gift. 
	replace year_from = "1979" if year_from == "1963" & tax == "gift" 
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/PK/Final_PK.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/PK/appended_PK.dta"	
