	use "$sources/Final_Data/HN/appended_HN", clear
	...
	// Overlapping between TIDD and EYa in 2008 for inheritance-everybody. TIDD here is superfluous for inheritance. 
	drop if Source == "TIDData" & year_from == "2008" & tax != "gift"
	
	// TIDD in conflict with EYa for gift in 2008!! We verified that EYa is wrong and corrected it at the source using IBFD
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/HN/Final_HN.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/HN/appended_HN.dta"	
