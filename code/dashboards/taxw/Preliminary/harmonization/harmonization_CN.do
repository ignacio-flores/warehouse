// Sources harmonization for China

	use "$sources/Final_Data/CN/appended_CN", clear
	...
	// Overlapping information of no estate, inheritance and gift tax from 2006 to 2013. The WashingtonLawreview2013 covers 1949-2013, so it's more comprehensive and we can eliminate EYa and EYb for the overlapping years.
	destring year_from, replace 
	drop if year_from >= 2006 & year_from <= 2013 
	tostring year_from, gen(pippo)
	drop year_from
	rename pippo year_from
	order GEO* curren year_from
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/CN/Final_CN.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/CN/appended_CN.dta"	