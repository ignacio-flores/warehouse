	use "$sources/Final_Data/HR/appended_HR", clear
	...
	// Overlapping between TIDD and EYa in 2006-2023 for inheritance-unknown. TIDD is useful from intro in 1993 to 2005, then EY has details of the tax scheme. 
	replace year_to = "2005" if year_to == "2021" & Source == "TIDData" & tax == "inheritance" & appl == "unknown"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/HR/Final_HR.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/HR/appended_HR.dta"	
