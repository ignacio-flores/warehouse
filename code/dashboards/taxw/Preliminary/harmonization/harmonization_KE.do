use "$sources/Final_Data/KE/appended_KE", clear
	...
	// Overlapping information between Kenya Law and EYa between 2006 and 2014. They both report status 0, we keep the law for source "superiority" and because of consistency of previous years coverage by the law 
	drop if substr(Source, 1,2)=="EY" & year_from == "2006" & year_to == "2014"
	...
	* No overlapping between children (or other) and everybody	
	...
	// Last possible overlapping: everybody and unknown
		...
		// Conflict between TID and legislation
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/KE/Final_KE.xlsx", firstrow(variables) sheet(Data) replace	
	erase "$sources/Final_Data/KE/appended_KE.dta"	
