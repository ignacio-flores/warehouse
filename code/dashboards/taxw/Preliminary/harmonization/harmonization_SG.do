use "$sources/Final_Data/SG/appended_SG", clear
		...
		// Overlapping between TIDD and EY for 2008. We drop TIDD 
		drop if year_from == "2008" & year_to == "2008" & tax == "inheritance" & applies_to == "everybody" & Source == "TIDData"
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
	...
	export excel using "$sources/Final_Data/SG/Final_SG.xlsx", firstrow(variables) sheet(Data) replace
	erase "$sources/Final_Data/SG/appended_SG.dta"
