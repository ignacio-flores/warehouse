use "$sources/Final_Data/KR/appended_KR", clear
	...
	// Conflict in the level of exemption between Yale and EYa. EYa reports the one that is also confirmed by IBFD, although EY reports (as in the note) that the basic allowance is or 500.000.000 or the sum of all deductions possible. IBFD does not report this sentence. Anyway, we keep EYa also because of the schedules 
	
		replace year_to = "2005" if year_to == "2008" & tax == "inheritance" & Source == "YaleInheritanceData"
		
	// Overlapping for estate-everybody between Yale and EY for 2006-2008. We keep Yale until 2005. 
		replace year_to = "2005" if year_to == "2008" & tax == "estate" & year_from == "1935" & Source == "YaleInheritanceData"
	...
	* No overlapping between children (or other) and everybody	
	...
	// Overlapping between TIDD (unknown) and Yale (children) for inheritance in 1947 AND 1968. We keep Yale as providing more info. 	
...
// Last check: conflict in status by tax -- we can have conflict between everybody vs any other kinship (BUT NOT BETWEEN ANY OTHER COMBINATION e.g. children vs spouse) 
		...
		export excel using "$sources/Final_Data/KR/Final_KR.xlsx", firstrow(variables) sheet(Data) replace	
		erase "$sources/Final_Data/KR/appended_KR.dta"	
