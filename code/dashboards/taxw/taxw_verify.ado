...
cap program drop taxw_verify
	program taxw_verify, nclass 
		version 16
	...
	display "Automatic fill if no tax or full exemption..."
		...
		// Check if the scheme is progressive (proportional, NOT by brackets)
		// If it is the case, set type of tax = 3
		if `nlb' > 1 { 
			qui egen xmin = rowmin(schedulelowerbounds2-schedulelowerbounds`nlb')
			qui egen xmax = rowmax(schedulelowerbounds2-schedulelowerbounds`nlb')
			qui replace typet = 3 if xmin == xmax & !missing(xmin) & status == 1 
			drop xmin xmax
	
			// Set to 1 the zeroes in case of progressive scheme
			forvalues i=2/`nlb' {
				qui replace schedulelowerbounds`i' = 1 if schedulelowerbounds`i' == 0
			}
		}
	...
	Therefore, cases where status is 0 don't have the flag generated in line 574 as marginalrates are 0 for status 0. I added the 
	...
	*/ 
	...
	restore 
		...
		*replace marginalrates0 = marginalrates1 if (marginalrates0 == . | marginalrates0 == 0) // to meet the same condition in line 569
		...
		replace schedulelowerbounds2 = . if round(schedulelowerbounds2, .01) == round(schedulelowerbounds1, .01) & marginalrates2 != -998 & (typetax != 3 | (typetax == 3 & exemption > 0))
		replace scheduleupperbounds2 = . if round(scheduleupperbounds2, .01) == round(scheduleupperbounds1, .01) & marginalrates2 != -998 & (typetax != 3 | (typetax == 3 & exemption > 0))
		replace marginalrates2 = . if round(marginalrates2, .01) == round(marginalrates1, .01) & marginalrates2 != -998	& (typetax != 3 | (typetax == 3 & exemption > 0))
		...
		forvalues i = 1/`mr' {
			...
			qui replace toprate = marginalrates`i' if toprate == . & marginalrates`j' == .
		}
		}
		...
		drop if schedulelowerbounds == . & bracket != 0	
		drop if scheduleupperbounds == . & bracket != 0	& typetax == 3
	...
	// Adjust the lower bounds to be 1 unit more than preceding upper bound
		...
		qui bys year* applies_to tax: ereplace exemption = min(exemption) 
		qui bys year* applies_to tax (bracket): replace schedulelowerbounds = schedulelowerbounds + 1 if bracket > 1 & typet == 3 & schedulelowerbounds == exemption // Progressive: all lower bounds are equal
		qui replace exemption = . if br != 0
		drop typet
		...
		// Apply the tax credit (cut the tax schedule there)
			...
			drop dupl exp
		// Adjust top rate
			qui bys year* tax applies_to (bracket): replace toprat = marginalrates[_N]
			replace toprat = . if bracket != 0
		}
	...
	// Drop if information is before the first year & set year_from to firstyear if it is lower! 
	qui drop if year_to < firsty
	qui replace year_from = firstyear if year_from < firstyear  // Case of Norway, for example!! 
