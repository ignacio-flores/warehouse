**********************************************************************
* TAX DUE FOR A $2M INHERITANCE (USD PPP, 2023)
**********************************************************************
...
*--------------------------------------------------------------------
	...
	gen equal_cs = abs(topayc-topays) < 1 
	gen equal_cl = abs(topayc-topayl) < 1 

	label var equal_cs "Child = Spouse"
	label var equal_cl "Child = Sibling"

	// Check numbers
	list GEO_long topayc topays topayl topayn, noobs sep(0)
	count if topayn !=0 // 37
	count if topayn ==0 // 125
	gen diff=topratn-topratc if topratn !=0 & topratn!=topratc
...
**********************************************************************
	...
	replace label = "" ///
		if (topays < 50000 & topays != 0 & topayc < 50000) ///
		| (topayc < 50000)

		gen obs  = 0 in 1
		replace obs = 660000 in 2

		gen obs2 = 0 in 1
		replace obs2 = 660000 in 2
		...
		ylab(0(60000)660000, nogrid angle(30) labsize(small) format(%8.0fc)) ///
		yscale(range(-30000 660000) noextend) ///
		xlab(0(100000)900000, nogrid angle(30) labsize(small) format(%8.0fc)) ///
		xscale(range(-50000 950000) noextend) ///
...
**********************************************************************
	...
	replace label = "" ///
		if (topayl < 50000 & topayl != 0 & topayc < 50000) ///
		| (topayc < 50000 & topayl < 100000)
...
**********************************************************************
	...
	graph combine spouse sibling, rows(1) ///
		title("Tax due in 2023 for a 2,000,000 USD inheritance, PPP-adjusted", ///
		size(small) box lcolor(black) margin(small)) ///
		subtitle(" ") ///
		imargins(2 2 2 2)
		
graph export "C:\Users\franc\Dropbox\gcwealth\raw_data\taxw\country_sheets\taxw_content.gph", as(png) name("Graph") replace
