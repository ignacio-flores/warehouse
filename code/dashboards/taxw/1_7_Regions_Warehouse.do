***********************************
*** taxw data: adjust for warehouse
***********************************

// Last update: July 2025
// Data used: $intfile/taxw_USstates_transformed.dta ; $hmade/dictionary.xlsx 
// Data output: $intfile/taxw_USstates_ready

	use "$intfile/taxw_USstates_transformed.dta", clear
...
// Remove labels 
	...
	qui sum bracket
	local max `r(max)'
	reshape wide adjlbo adjubo adjmrt status exempt toprat toplbo revenu prorev revgdp typtax, i(GeoReg year d2) j(bracket)

	foreach var in adjlbo adjubo adjmrt status exempt toprat toplbo revenu prorev revgdp typtax {
		forvalues i = 0/`max' {
...
// d3_vartype
	...
	gen d3 = "cat" if substr(concept, 1, 6) == "status" | substr(concept, 1, 6) == "homexe" ///
...
// Save and export 
	...
	rename taxnote note
	keep GEO* Geo* year value percentile varcode source source_legend longname note
	label var source_legend ""
	label var source ""
	label var note ""
	qui save "$intfile/taxw_USstates_ready.dta", replace
