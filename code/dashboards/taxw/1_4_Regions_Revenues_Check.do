...
************************* 1. Regions codes *************************************
	preserve 
		qui qui import excel "$hmade\dictionary.xlsx", sheet("GEO") cellrange(A1:C500) firstrow clear
	...
	preserve 
		qui import excel "$hmade\dictionary.xlsx", sheet("GEOReg") cellrange(A1:C500) firstrow clear
		rename Country GEO 
...
// Package required, automatic check 
	...
	qui save "$intfile/taxw_USstates_revenue_data.dta", replace
